...
TEST(int32)
{
    ...
    for (i = 0; i < membersof(datas); i++) {
        ...
        ASSERT_MEMORY_EQ(&encoded[0], datas[i].encoded_p, size);
...
TEST(int32_message_2)
{
    ...
    ASSERT_EQ(size, 3);
    ASSERT_MEMORY_EQ(&encoded[0], encoded_p, size);
...
TEST(bytes)
{
    ...
    for (i = 0; i < membersof(datas); i++) {
        ...
        ASSERT_MEMORY_EQ(message_p->value.buf_p,
                      datas[i].decoded_p,
...
TEST(address_book_default_person)
{
    ...
    size = address_book_address_book_encode(address_book_p,
                                            ...
                                            sizeof(encoded));
    ASSERT_EQ(size, 2);
    ASSERT_MEMORY_EQ(&encoded[0], "\x0a\x00", size);
...
TEST(tags_1)
{
    ...
    ASSERT_EQ(size, 2);
    ASSERT_MEMORY_EQ(&encoded[0], "\x08\x01", size);
...
TEST(tags_2)
{
    ...
    ASSERT_EQ(size, 2);
    ASSERT_MEMORY_EQ(&encoded[0], "\x78\x01", size);
...
TEST(tags_3)
{
    ...
    ASSERT_EQ(size, 3);
    ASSERT_MEMORY_EQ(&encoded[0], "\x80\x01\x01", size);
...
TEST(tags_4)
{
    ...
    ASSERT_EQ(size, 3);
    ASSERT_MEMORY_EQ(&encoded[0], "\xf8\x7f\x01", size);
...
TEST(tags_5)
{
    ...
    ASSERT_EQ(size, 4);
    ASSERT_MEMORY_EQ(&encoded[0], "\x80\x80\x01\x01", size);
...
TEST(tags_6)
{
    ...
    ASSERT_EQ(size, 6);
    ASSERT_MEMORY_EQ(&encoded[0], "\xf8\xff\xff\xff\x0f\x01", size);
...
TEST(oneof_message_v1)
{
    ...
    ASSERT_NE(message_p, NULL);
    ASSERT_EQ(message_p->value, oneof_message_value_none_e);

    oneof_message_v1_init(message_p);
    message_p->v1 = 65;
    size = oneof_message_encode(message_p, &encoded[0], sizeof(encoded));
    ASSERT_EQ(size, 2);
    ASSERT_MEMORY_EQ(&encoded[0], "\x08\x41", size);

    message_p = oneof_message_new(&workspace[0], sizeof(workspace));
    ASSERT_NE(message_p, NULL);
    size = oneof_message_decode(message_p, &encoded[0], size);
    ASSERT_EQ(size, 2);
    ASSERT_EQ(message_p->value, oneof_message_value_v1_e);
    ASSERT_EQ(message_p->v1, 65);
}

TEST(oneof_message_v1_default_value)
{
    uint8_t encoded[128];
    int size;
    uint8_t workspace[512];
    ...
    size = oneof_message_encode(message_p, &encoded[0], sizeof(encoded));
    ASSERT_EQ(size, 2);
    ASSERT_MEMORY_EQ(&encoded[0], "\x08\x00", size);
...
TEST(oneof_message_v2)
{
    ...
    message_p = oneof_message_new(&workspace[0], sizeof(workspace));
    ASSERT_NE(message_p, NULL);
    oneof_message_v2_init(message_p);
    message_p->v2_p = "Hello!";
    size = oneof_message_encode(message_p, &encoded[0], sizeof(encoded));
    ASSERT_EQ(size, 8);
    ASSERT_MEMORY_EQ(&encoded[0], "\x12\x06Hello!", size);

    message_p = oneof_message_new(&workspace[0], sizeof(workspace));
    ASSERT_NE(message_p, NULL);
    size = oneof_message_decode(message_p, &encoded[0], size);
    ASSERT_EQ(size, 8);
    ASSERT_EQ(message_p->value, oneof_message_value_v2_e);
    ASSERT_EQ(message_p->v2_p, "Hello!");
}

TEST(oneof_message_v2_default_value)
{
    uint8_t encoded[128];
    int size;
    uint8_t workspace[512];
    ...
    size = oneof_message_encode(message_p, &encoded[0], sizeof(encoded));
    ASSERT_EQ(size, 2);
    ASSERT_MEMORY_EQ(&encoded[0], "\x12\x00", size);
...
TEST(oneof_message2_v6)
{
    ...
    ASSERT_EQ(size, 4);
    ASSERT_MEMORY_EQ(&encoded[0], "\x08\x01\x30\x01", size);
...
TEST(oneof_message2_v5)
{
    ...
    message_p = oneof_message2_new(&workspace[0], sizeof(workspace));
    ASSERT_NE(message_p, NULL);
    oneof_message2_v5_init(message_p);
    message_p->v5.buf_p = (uint8_t *)"123";
    message_p->v5.size = 3;
    ASSERT_EQ(oneof_message2_v2_alloc(message_p), 0);
    message_p->v2_p->bar = 9999;
    ...
    ASSERT_EQ(size, 11);
    ASSERT_EQ(message_p->oneof1, oneof_message2_oneof1_v5_e);
    ASSERT_EQ(message_p->v5.size, 3);
    ASSERT_MEMORY_EQ(message_p->v5.buf_p, "123", 3);
    ASSERT_EQ(message_p->oneof2, oneof_message2_oneof2_v2_e);
    ASSERT_EQ(message_p->v2_p->bar, 9999);
}

TEST(oneof_message2_v1_default_value)
{
    uint8_t encoded[128];
    int size;
    uint8_t workspace[512];
    ...
    ASSERT_EQ(size, 2);
    ASSERT_EQ(message_p->oneof2, oneof_message2_oneof2_v1_e);
    ASSERT_FALSE(message_p->v1);
}

TEST(oneof_message2_v2_default_value)
{
    uint8_t encoded[128];
    int size;
    uint8_t workspace[512];
    ...
    ASSERT_EQ(size, 2);
    ASSERT_EQ(message_p->oneof2, oneof_message2_oneof2_v2_e);
    ASSERT_EQ(message_p->v2_p->bar, 0);
}

TEST(oneof_message2_v3_default_value)
{
    uint8_t encoded[128];
    int size;
    uint8_t workspace[512];
    ...
    message_p = oneof_message2_new(&workspace[0], sizeof(workspace));
    ASSERT_NE(message_p, NULL);
    ASSERT_EQ(oneof_message2_v3_alloc(message_p), 0);

    size = oneof_message2_encode(message_p, &encoded[0], sizeof(encoded));
    ASSERT_EQ(size, 2);
    ASSERT_MEMORY_EQ(&encoded[0], "\x1a\x00", size);
    ...
    ASSERT_EQ(size, 2);
    ASSERT_EQ(message_p->oneof2, oneof_message2_oneof2_v3_e);
    ASSERT_EQ(message_p->v3_p->value, oneof_message_value_none_e);
}

TEST(oneof_message2_v3_v1_default_value)
{
    uint8_t encoded[128];
    int size;
    uint8_t workspace[512];
    ...
    ASSERT_NE(message_p, NULL);
    ASSERT_EQ(oneof_message2_v3_alloc(message_p), 0);
    oneof_message_v1_init(message_p->v3_p);

    size = oneof_message2_encode(message_p, &encoded[0], sizeof(encoded));
    ASSERT_EQ(size, 4);
    ASSERT_MEMORY_EQ(&encoded[0], "\x1a\x02\x08\x00", size);
    ...
    ASSERT_EQ(size, 4);
    ASSERT_EQ(message_p->oneof2, oneof_message2_oneof2_v3_e);
    ASSERT_EQ(message_p->v3_p->value, oneof_message_value_v1_e);
    ASSERT_EQ(message_p->v3_p->v1, 0);
}

TEST(oneof_message2_v4_default_value)
{
    uint8_t encoded[128];
    int size;
    uint8_t workspace[512];
    ...
    message_p = oneof_message2_new(&workspace[0], sizeof(workspace));
    ASSERT_NE(message_p, NULL);
    ASSERT_EQ(oneof_message2_v4_alloc(message_p), 0);

    size = oneof_message2_encode(message_p, &encoded[0], sizeof(encoded));
    ASSERT_EQ(size, 2);
    ASSERT_MEMORY_EQ(&encoded[0], "\x22\x00", size);
    ...
    ASSERT_EQ(size, 2);
    ASSERT_EQ(message_p->oneof1, oneof_message2_oneof1_v4_e);
    ASSERT_EQ(message_p->v4_p->bar, 0);
}

TEST(oneof_message2_v5_default_value)
{
    uint8_t encoded[128];
    int size;
    uint8_t workspace[512];
    ...
    size = oneof_message2_encode(message_p, &encoded[0], sizeof(encoded));
    ASSERT_EQ(size, 2);
    ASSERT_MEMORY_EQ(&encoded[0], "\x2a\x00", size);
    ...
    ASSERT_EQ(size, 2);
    ASSERT_EQ(message_p->oneof1, oneof_message2_oneof1_v5_e);
    ASSERT_EQ(message_p->v5.size, 0);
}

TEST(oneof_message2_v6_default_value)
{
    uint8_t encoded[128];
    int size;
    uint8_t workspace[512];
...
TEST(oneof_message3)
{
    ...
    ASSERT_NE(message_p, NULL);
    ASSERT_EQ(oneof_message3_v1_alloc(message_p), 0);
    ASSERT_EQ(oneof_message3_bar_foo_alloc(message_p->v1_p, 2), 0);

    foo_p = &message_p->v1_p->foo.items_p[0];
    oneof_message3_foo_v2_init(foo_p);
    foo_p->v2.buf_p = (uint8_t *)"456";
    foo_p->v2.size = 3;

    foo_p = &message_p->v1_p->foo.items_p[1];
    oneof_message3_foo_v1_init(foo_p);
    foo_p->v1 = true;
    ...
    ASSERT_EQ(size, 13);
    ASSERT_EQ(message_p->oneof1, oneof_message3_oneof1_v1_e);
    ASSERT_EQ(message_p->v1_p->foo.length, 2);

    foo_p = &message_p->v1_p->foo.items_p[0];
    ASSERT_EQ(foo_p->inner_oneof, oneof_message3_foo_inner_oneof_v2_e);
    ASSERT_EQ(foo_p->v2.size, 3);
    ASSERT_MEMORY_EQ(foo_p->v2.buf_p, "456", 3);

    foo_p = &message_p->v1_p->foo.items_p[1];
    ASSERT_EQ(foo_p->inner_oneof, oneof_message3_foo_inner_oneof_v1_e);
    ASSERT_EQ(foo_p->v1, true);
}
...
TEST(repeated_int32s_one_item)
{
    ...
    ASSERT_EQ(size, 3);
    ASSERT_MEMORY_EQ(&encoded[0], "\x0a\x01\x07", size);
...
TEST(repeated_int32s_two_items)
{
    ...
    size = repeated_message_encode(message_p, &encoded[0], sizeof(encoded));
    ASSERT_EQ(size, 5);
    ASSERT_MEMORY_EQ(&encoded[0], "\x0a\x03\x01\xe8\x07", size);
...
TEST(repeated_bytes_empty_item)
{
    ...
    size = repeated_message_encode(message_p, &encoded[0], sizeof(encoded));
    ASSERT_EQ(size, 5);
    ASSERT_MEMORY_EQ(&encoded[0], "\x22\x00\x22\x01\x32", size);
    ...
    ASSERT_MEMORY_EQ(message_p->bytes.items_p[1].buf_p, "2", 1);
}
...
TEST(repeated_bytes_two_items)
{
    ...
    size = repeated_message_encode(message_p, &encoded[0], sizeof(encoded));
    ASSERT_EQ(size, 6);
    ASSERT_MEMORY_EQ(&encoded[0], "\x22\x01\x31\x22\x01\x32", size);
    ...
    ASSERT_MEMORY_EQ(message_p->bytes.items_p[0].buf_p, "1", 1);
    ASSERT_EQ(message_p->bytes.items_p[1].size, 1);
    ASSERT_MEMORY_EQ(message_p->bytes.items_p[1].buf_p, "2", 1);
}
...
TEST(repeated_scalar_value_types)
{
    ...
    ASSERT_MEMORY_EQ(message_p->bytess.items_p[0].buf_p, "17", 2);
}
...
TEST(repeated_scalar_value_types_bool)
{
    ...
    size = repeated_message_scalar_value_types_encode(message_p,
                                                      ...
                                                      sizeof(encoded));
    ASSERT_EQ(size, 4);
    ASSERT_MEMORY_EQ(&encoded[0], "\x6a\x02\x01\x00", 4);
...
TEST(repeated_scalar_value_types_string_empty_item)
{
    ...
    size = repeated_message_scalar_value_types_encode(message_p,
                                                      ...
                                                      sizeof(encoded));
    ASSERT_EQ(size, 5);
    ASSERT_MEMORY_EQ(&encoded[0], "\x72\x00\x72\x01\x31", 5);
...
TEST(repeated_scalar_value_types_sint32)
{
    ...
    size = repeated_message_scalar_value_types_encode(message_p,
                                                      ...
                                                      sizeof(encoded));
    ASSERT_EQ(size, 5);
    ASSERT_MEMORY_EQ(&encoded[0], "\x1a\x03\x00\x09\x0a", 5);
...
TEST(repeated_scalar_value_types_sint64)
{
    ...
    size = repeated_message_scalar_value_types_encode(message_p,
                                                      ...
                                                      sizeof(encoded));
    ASSERT_EQ(size, 6);
    ASSERT_MEMORY_EQ(&encoded[0], "\x22\x04\xd6\x08\x00\x01", 6);
...
TEST(repeated_scalar_value_types_int32_two_default_items)
{
    ...
    size = repeated_message_scalar_value_types_encode(message_p,
                                                      ...
                                                      sizeof(encoded));
    ASSERT_EQ(size, 4);
    ASSERT_MEMORY_EQ(&encoded[0], "\x0a\x02\x00\x00", 4);
...
TEST(repeated_foo_enums)
{
    ...
    ASSERT_EQ(size, 4);
    ASSERT_MEMORY_EQ(&encoded[0], "\x12\x02\x00\x01", 4);
...
TEST(message_decode_error_bad_sub_message_wire_type)
{
    ...
    message_p = message_message_new(&workspace[0], sizeof(workspace));
    ASSERT_NE(message_p, NULL);
    size = message_message_decode(message_p,
                                  (const uint8_t *)"\x81\x34\x03\xe8\x14\x01",
                                  6);
    ASSERT_EQ(size, -PBTOOLS_BAD_WIRE_TYPE);
}
...
TEST(no_package)
{
    ...
    ASSERT_EQ(size, 8);
    ASSERT_MEMORY_EQ(&encoded[0], "\x0a\x02\x08\x01\x12\x00\x18\x01", size);
...
TEST(importing_message)
{
    ...
    ASSERT_EQ(size, 6);
    ASSERT_MEMORY_EQ(&encoded[0], "\x08\x01\x12\x02\x08\x01", size);
...
TEST(importing_message_same_package)
{
    ...
    size = importing_message_encode(message_p, &encoded[0], sizeof(encoded));
    ASSERT_EQ(size, 7);
    ASSERT_MEMORY_EQ(&encoded[0], "\x12\x05\x12\x03\x98\x06\x01", size);
...
TEST(map_message_encode_decode)
{
    ...
    ASSERT_EQ(message_p->map2.items_p[0].value_p->v1, true);
}
...
TEST(add_and_remove_fields_version1)
{
    ...
    size = add_and_remove_fields_version1_encode(message_p,
                                                 ...
                                                 sizeof(encoded));
    ASSERT_EQ(size, 2);
    ASSERT_MEMORY_EQ(&encoded[0], "\x08\x05", size);

    /* Decode Version2 as Version1. */
    message_p = add_and_remove_fields_version1_new(&workspace[0], sizeof(workspace));
    ASSERT_NE(message_p, NULL);
    size = add_and_remove_fields_version1_decode(
        message_p,
        (const uint8_t *)"\x08\x05\x10\x06\x18\x07",
        6);
    ASSERT_EQ(size, 6);
...
TEST(add_and_remove_fields_version3)
{
    ...
    size = add_and_remove_fields_version3_encode(message_p,
                                                 ...
                                                 sizeof(encoded));
    ASSERT_EQ(size, 4);
    ASSERT_MEMORY_EQ(&encoded[0], "\x18\x07\x20\x08", size);
    ...
    message_p = add_and_remove_fields_version3_new(&workspace[0], sizeof(workspace));
    ASSERT_NE(message_p, NULL);
    size = add_and_remove_fields_version3_decode(
        message_p,
        (const uint8_t *)"\x08\x05\x10\x06\x18\x07",
        6);
    ASSERT_EQ(size, 6);
